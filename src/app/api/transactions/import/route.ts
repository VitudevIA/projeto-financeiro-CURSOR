/**
 * API: Importa├º├úo de Transa├º├Áes
 * POST /api/transactions/import
 * 
 * Importa transa├º├Áes de um arquivo CSV ou XLSX
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (fileName.endsWith('.csv')) {
      ...
      // Para XLSX, seria necess├írio usar uma biblioteca server-side
      // Por enquanto, retornamos erro informando que CSV ├® preferido
      return NextResponse.json(
        { error: 'Formato XLSX requer processamento adicional. Use CSV ou converta o arquivo.' },
        { status: 400 }
      )
    } else {
      return NextResponse.json({ error: 'Formato de arquivo n├úo suportado' }, { status: 400 })
    }

    if (transactions.length === 0) {
      return NextResponse.json({ error: 'Nenhuma transa├º├úo encontrada no arquivo' }, { status: 400 })
    }

    // Busca categorias do usu├írio para mapear
    ...
    // Processa e insere transa├º├Áes
    let successCount = 0
    let errorCount = 0
    const errors: string[] = []
    ...
    for (const transaction of transactions) {
      try {
        // Valida e processa transa├º├úo
        if (!transaction.data || !transaction.descricao || !transaction.valor) {
          errorCount++
          errors.push(`Linha inv├ílida: ${transaction.descricao || 'sem descri├º├úo'}`)
          continue
        }
        ...
        if (isNaN(amount) || amount <= 0) {
          errorCount++
          errors.push(`Valor inv├ílido para: ${transaction.descricao}`)
          continue
        }
        ...
        if (transaction.categoria) {
          ...
          // Se n├úo encontrar, cria uma nova categoria
          if (!categoryId) {
        ...
        // Se ainda n├úo tem categoria, busca primeira dispon├¡vel ou cria padr├úo
        if (!categoryId) {
        ...
        console.error('Erro ao importar transa├º├úo:', error)
      }
    }
    ...
    return NextResponse.json(
      {
        ...
        message: `${successCount} transa├º├úo(├Áes) importada(s) com sucesso${errorCount > 0 ? `, ${errorCount} erro(s)` : ''}`,
      },
      { status: 200 }
    )
  } catch (error) {
    console.error('Erro ao importar transa├º├Áes:', error)
    return NextResponse.json(
      { error: 'Erro interno do servidor ao importar transa├º├Áes' },
      { status: 500 }
    )
  }
}
