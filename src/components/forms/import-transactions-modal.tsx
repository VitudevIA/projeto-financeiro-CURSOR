...
export function ImportTransactionsModal({
  ...
  const handleDownloadTemplate = (format: 'csv' | 'xlsx') => {
    try {
      ...
      toast.error('Erro ao baixar modelo de importa├º├úo')
    }
  }
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isValidFormat) {
      toast.error('Formato inv├ílido. Use CSV ou XLSX.')
      return
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Erro ao importar transa├º├Áes')
      }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Importe m├║ltiplas despesas de uma vez usando uma planilha CSV ou XLSX
          </DialogDescription>
        </DialogHeader>

        <div className="space-y-6 py-4">
          {/* Op├º├úo 1: Baixar Modelo */}
          <div className="border rounded-lg p-4 space-y-4">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <h3 className="font-semibold mb-1">Modelo de Importa├º├úo</h3>
          ...
          <div className="bg-muted/50 rounded-lg p-4 space-y-2">
            <h4 className="font-semibold text-sm">Instru├º├Áes:</h4>
            <ul className="text-sm text-muted-foreground space-y-1 list-disc list-inside">
              <li>Baixe o modelo e preencha com suas despesas</li>
              <li>Mantenha o formato original (n├úo altere as colunas)</li>
              <li>Data no formato: AAAA-MM-DD (ex: 2025-01-15)</li>
              <li>Valor em n├║mero decimal (ex: 150.50)</li>
              <li>M├®todo de pagamento: credit, debit, cash, pix ou boleto</li>
              <li>As linhas com dados inv├ílidos ser├úo ignoradas</li>
            </ul>
          </div>
        </div>
